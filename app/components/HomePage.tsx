...
export function HomePage({ locale }: { locale: Locale }) {
  ...
  return <><SiteHeader locale={locale} route="home"/><main id="main-content">
    <section className="home-hero"><div className="hero-grid"/><div className="hero-orbit hero-orbit--one"/><div className="hero-orbit hero-orbit--two"/><div className="hero-content shell-wide"><HazardEmblem large/><p className="eyebrow"><span>◈</span>{c.eyebrow}<span>◈</span></p><h1>{c.titlePrefix}<span>{c.titleGame}</span></h1><p className="hero-lead">{c.lead}</p><div className="hero-actions"><Link className="button button--gold" href={pathFor(locale,"how-to-play")}>{c.primary}</Link><Link className="button button--outline" href={`${prefix}/minigames/`}>{c.secondary}</Link></div></div></section>
    ...
    <section className="home-section shell-wide"><Heading kicker={c.statusKicker} title={c.statusTitle} deck={c.statusDeck}/><div className="status-grid">{c.statuses.map(([tone,label,title,body]) => <article key={title} className={`status-card status-card--${tone}`}><span>{label}</span><h3>{title}</h3><p>{body}</p></article>)}</div><div className="inline-links"><Link href={`${prefix}/guides/online-multiplayer/`}>{c.statusLinks[0]}</Link><Link href={`${prefix}/guides/local-couch-coop/`}>{c.statusLinks[1]}</Link><Link href={`${prefix}/guides/max-players/`}>{c.statusLinks[2]}</Link></div></section>
    <section className="home-section home-section--tinted"><div className="shell-wide split-feature"><div className="simulation-visual"><span>15</span><small>{c.archiveLabel}</small></div><div><Heading kicker={c.archiveKicker} title={c.archiveTitle} deck={c.archiveDeck}/><Link className="button button--gold" href={`${prefix}/minigames/`}>{c.archiveButton}</Link></div></div></section>
    <section className="home-section shell-wide"><Heading center kicker={c.everyKicker} title={c.everyTitle}/><div className="guide-grid guide-grid--three">{cards.map(card => <GuideCard key={card.title} locale={locale} openLabel={copy[locale].guides.open} {...card}/>)}</div><div className="center-link"><Link className="text-link" href={pathFor(locale,"guides")}>{c.viewGuides} →</Link></div></section>
    ...
    <section className="home-cta"><div className="shell-wide"><p>{c.ctaKicker}</p><h2>{c.ctaTitle}</h2><div className="hero-actions"><Link className="button button--gold" href={pathFor(locale,"guides")}>{c.ctaPrimary}</Link><a className="button button--outline" href={officialLinks.steam}>{c.ctaSecondary}</a></div></div></section>
