...
function LanguageSwitcher({ locale, route }: { locale: Locale; route: RouteKey }) {
  return <div className="language-switcher" role="group" aria-label="Select language"><details><summary><span>{localeNames[locale]}</span><span aria-hidden="true">▾</span></summary><div className="language-switcher__options">{locales.map(item => <a key={item} href={pathFor(item, route)} aria-current={item === locale ? "page" : undefined}>{localeNames[item]}</a>)}</div></details></div>;
}
  ...
  return <>
    ...
    <header className="site-header">
      <div className="header-primary shell-wide">
        <a className="brand" aria-label="Machine Party Home" href={pathFor(locale, "home")}><HazardEmblem/><span><strong>Machine Party</strong><small>{c.brandTag}</small></span></a>
        <a className="steam-button" href={officialLinks.steam}> {c.steam}</a>
        <nav className="desktop-primary-nav" aria-label="Primary navigation">{c.nav.map((label, i) => <a key={label} href={links[i]}>{label}</a>)}</nav>
        <div className="desktop-language"><LanguageSwitcher locale={locale} route={route}/></div>
        <div className="mobile-actions"><LanguageSwitcher locale={locale} route={route}/><div className="mobile-menu"><button type="button" className="menu-toggle" aria-expanded={open} aria-controls="mobile-navigation" aria-label="Open navigation" onClick={() => setOpen(v => !v)}><span/><span/></button>{open && <nav id="mobile-navigation">{c.nav.map((label, i) => <a key={label} href={links[i]} onClick={() => setOpen(false)}>{label}</a>)}</nav>}</div></div>
      </div>
      <div className="header-secondary"><nav className="shell-wide" aria-label="Guides">{c.subnav.map((label, i) => <a key={label} href={sublinks[i]}>{label}</a>)}</nav></div>
    </header>
  </>;
}
...
export function SiteFooter({ locale }: { locale: Locale }) {
  ...
  return <footer className="site-footer"><div className="footer-grid shell-wide"><div className="footer-brand"><a className="brand" href={`${prefix}/`}><HazardEmblem/><span><strong>Machine Party</strong><small>Independent guide · 2026</small></span></a><p>{c.footer.disclaimer}</p><a className="footer-steam" href={officialLinks.steam}>{c.footer.buy} →</a></div><FooterGroup title={c.footer.groups[0]} links={[[l[0],`${prefix}/guides/how-to-play/`],[l[1],`${prefix}/guides/`],[l[2],`${prefix}/minigames/`]]}/><FooterGroup title={c.footer.groups[1]} links={[[l[3],officialLinks.website],[l[4],officialLinks.discord],[l[5],officialLinks.youtube]]}/><FooterGroup title={c.footer.groups[2]} links={[[l[6],`${prefix}/about/`],[l[7],`${prefix}/contact/`],[l[8],`${prefix}/privacy/`],[l[9],`${prefix}/terms/`]]}/></div><section className="native-ad-banner" aria-label={c.footer.ad}><p>{c.footer.ad}</p><div className="native-ad-banner__slot"/></section><div className="footer-bottom shell-wide"><span>© 2026 Machine Party Independent Guide</span><span>{localeNames[locale]}</span></div></footer>;
}

function FooterGroup({ title, links }: { title: string; links: string[][] }) { return <div className="footer-group"><h2>{title}</h2><ul>{links.map(([label, href]) => <li key={label}><a href={href}>{label}</a></li>)}</ul></div>; }
