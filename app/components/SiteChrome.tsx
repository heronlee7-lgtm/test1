...
import { copy, localeNames, locales, pathFor, type Locale, type RouteKey } from "./i18n";

const steamUrl = "https://store.steampowered.com/app/4108000/Machine_Party/";
  ...
  return <>
    ...
    <header className="site-header">
      <div className="header-primary shell-wide">
        ...
        <a className="steam-button" href={steamUrl}> {c.steam}</a>
...
export function SiteFooter({ locale }: { locale: Locale }) {
  const c = copy[locale];
  const prefix = locale === "en" ? "" : `/${locale}`;
  return <footer className="site-footer"><div className="footer-grid shell-wide"><div className="footer-brand"><Link className="brand" href={`${prefix}/`}><HazardEmblem/><span><strong>Machine Party</strong><small>Wiki · 2026</small></span></Link><p>{c.footer.disclaimer}</p><a className="footer-steam" href={steamUrl}>{c.footer.buy} →</a></div><FooterGroup title={c.footer.groups[0]} links={[["How to Play",`${prefix}/guides/how-to-play/`],["Max Players",`${prefix}/guides/max-players/`],["Minigames",`${prefix}/minigames/`]]}/><FooterGroup title={c.footer.groups[1]} links={[["Multiplayer",`${prefix}/guides/online-multiplayer/`],["Local Co-op",`${prefix}/guides/local-couch-coop/`],["Platforms",`${prefix}/platforms/`]]}/><FooterGroup title={c.footer.groups[2]} links={[["About",`${prefix}/about/`],["Contact",`${prefix}/contact/`],["Privacy",`${prefix}/privacy/`],["Terms",`${prefix}/terms/`]]}/></div><section className="native-ad-banner" aria-label="Advertisement"><p>Advertisement</p><div className="native-ad-banner__slot"/></section><div className="footer-bottom shell-wide"><span>© 2026 Machine Party Wiki</span><span>{localeNames[locale]}</span></div></footer>;
}
